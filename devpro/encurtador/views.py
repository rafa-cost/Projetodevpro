...
def relatorios(requisicao, slug):
    ...
    url_reduzida = requisicao.build_absolute_uri(f'/{slug}')  #aqui é o http://127.0.0.1:8000/slug do site, nesta variavel conta a quantidade de cliques. "requisicao.build_absolute_uri" isso aqui constrói a url completa com esquema e o dominio. Ela recebe como parametro o path que eu quero acrescentar aqui (f'/{slug}')
    #ele fez esse código abaixo pelo debugger na parte do console
    redirecionamentos_por_data = list(
        UrlRedirect.objects.filter(
            slug=slug
        ).annotate(
            data=TruncDate('logs__criado_em')         #"logs" é a referencia que faz a ligação entre UrlRedirect e UrlLogdo. Esse TruncDate ira só pegar a data do  "criado_em". Bem dizer aqui indica o endereço do site e os dias dos cliques.
        ).annotate(
    ...
    contexto = {'reduce': url_redirect, 'url_reduzida': url_reduzida,
                ...
                'total_cliques': sum(r.cliques for r in redirecionamentos_por_data)   #aqui esta somando a quantidade total de cliques e passando para variavel "total_cliques" do html
                }
